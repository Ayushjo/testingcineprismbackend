...
const fetchUnsplashImage = async (searchQuery) => {
    try {
        ...
        if (!cleanQuery.includes("entertainment") &&
...
const getArticleImage = async (article) => {
...
const extractSearchTerms = (article) => {
    const terms = [];
    ...
    if (content.includes("marvel"))
        terms.push("marvel superhero movie");
    if (content.includes("disney"))
        terms.push("disney movie");
    if (content.includes("netflix"))
        terms.push("netflix streaming");
    if (content.includes("oscar") || content.includes("academy"))
        terms.push("oscar academy awards");
    if (content.includes("box office"))
        terms.push("movie box office cinema");
    if (content.includes("premiere"))
        terms.push("movie premiere red carpet");
    if (content.includes("trailer"))
        terms.push("movie trailer cinema");
    // Generic fallback
    terms.push("entertainment news cinema");
    return terms;
};
// RSS Feed Scraping
const scrapeFromRSS = async (feedUrl, sourceName) => {
    try {
...
const scrapeFullArticle = async (articleUrl) => {
    try {
    ...
    try {
        ...
        $(source.selector.container).each((index, element) => {
            ...
            try {
                ...
                console.warn(`Error parsing article from ${source.name}:`, error.message);
            }
        });
        const validArticles = articles.filter((article) => article.title.length > 15 &&
            article.url.startsWith("http") &&
            !article.title.toLowerCase().includes("404"));
        console.log(`Web Scraping: Found ${validArticles.length} articles from ${source.name}`);
...
const refreshTrendingNews = async (req, res) => {
    ...
    try {
        ...
        for (const feed of newsSources_1.rssFeeds) {
            try {
        ...
        for (const source of newsSources_1.newsSources) {
            try {
                const scrapedNews = await scrapeNewsSource(source);
                // Enhance top 3 articles from each source
                const enhancedNews = await Promise.all(scrapedNews.slice(0, 3).map(async (article) => {
        ...
        const articlesWithImages = await Promise.all(uniqueArticles.map(async (article) => {
        ...
        res.status(200).json({
            ...
            data: {
                ...
                unsplash_images_used: topNews.filter((a) => a.image_url && a.image_url.includes("unsplash")).length,
            },
        });
    }
    catch (error) {
    ...
    try {
        ...
        if (!imageUrl || imageUrl.includes("placeholder")) {
            ...
            if (imageUrl && imageUrl !== article.imageUrl) {
                await __1.default.trendingNews.update({
                    where: { id: article.id },
                    data: { imageUrl },
                });
            }
