...
async function main() {
    ...
    const { emailWorker, emailWorkerConnection } = await import("../../src/queues/emailWorker.js");
    ...
    const resultPromise = new Promise((resolve) => {
        ...
        const onCompleted = ({ jobId: completedId }) => {
    ...
    if (result === "completed") {
        ...
        pass("Unsubscribe link should point to FRONTEND_URL/unsubscribe?token=test-token-123");
    }
    else if (result === "failed") {
        fail("Worker failed to send the email — check AWS SES credentials and FROM address");
        process.exitCode = 1;
    }
    else {
