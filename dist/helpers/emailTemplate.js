...
exports.buildCampaignEmail = buildCampaignEmail;
...
`.replace(/\s+/g, " ").trim();
const LOGO_URL = "https://thecineprismimages.s3.ap-south-1.amazonaws.com/articles/main-images/thecineprismlogo.jpg";
const masthead = (preheader) => `
...
`;
const sep = () => `<tr><td style="padding:0 40px;"><div style="border-bottom:1px solid #e2e8f0;"></div></td></tr>`;
// ── Campaign email ────────────────────────────────────────────────────────────
function buildCampaignEmail(content, previewText, subject) {
    const preview = previewText || subject || "Your curated film digest is here.";
...
`;
    ...
    if (content.comingNextWeek?.length) {
        const items = content.comingNextWeek
            .map((item) => `<li style="margin-bottom:12px;padding-left:8px;font-size:14px;line-height:1.5;">${item}</li>`)
            .join("");
        html += `
      <tr>
        <td style="padding:40px 40px;">
