  ...
  for (const article of articles) {
    try {
      ...
      if (article.mainImageUrl) {
        ...
        const fileName = `articles/main-images/${
          article.id
        }-${getFileNameFromUrl(article.mainImageUrl)}`;
        const newUrl = await uploadToS3(
          imageBuffer,
          fileName,
          getContentType(article.mainImageUrl)
        );
      ...
      for (const block of article.blocks) {
        if (block.type === "IMAGE" && block.content) {
          ...
          if (content.url) {
            ...
            const fileName = `articles/content-blocks/${article.id}-${
              block.id
            }-${getFileNameFromUrl(content.url)}`;
            const newUrl = await uploadToS3(
              imageBuffer,
              fileName,
              getContentType(content.url)
            );
            ...
            console.log(
              `✅ Migrated block image for article: ${article.title}`
            );
          }
        }
      }
  ...
  for (const post of posts) {
    try {
      ...
      if (post.posterImageUrl) {
        ...
        const fileName = `posts/posters/${post.id}-${getFileNameFromUrl(
          post.posterImageUrl
        )}`;
        const newUrl = await uploadToS3(
          imageBuffer,
          fileName,
          getContentType(post.posterImageUrl)
        );
      ...
      if (post.reviewPosterImageUrl) {
        ...
        const fileName = `posts/review-posters/${post.id}-${getFileNameFromUrl(
          post.reviewPosterImageUrl
        )}`;
        const newUrl = await uploadToS3(
          imageBuffer,
          fileName,
          getContentType(post.reviewPosterImageUrl)
        );
      ...
      for (const image of post.images) {
        ...
        const fileName = `posts/images/${post.id}-${
          image.id
  ...
  for (const genre of genres) {
    try {
      if (genre.posterImageUrl) {
        ...
        const fileName = `by-genres/${genre.id}-${getFileNameFromUrl(
          genre.posterImageUrl
        )}`;
        const newUrl = await uploadToS3(
          imageBuffer,
          fileName,
          getContentType(genre.posterImageUrl)
        );
      ...
      console.error(
        `❌ Error migrating byGenres ${genre.title}:`,
        error.message
      );
      errorCount++;
    }
  ...
  for (const pick of picks) {
    try {
      if (pick.posterImageUrl) {
        ...
        const fileName = `top-picks/${pick.id}-${getFileNameFromUrl(
          pick.posterImageUrl
        )}`;
        const newUrl = await uploadToS3(
          imageBuffer,
          fileName,
          getContentType(pick.posterImageUrl)
        );
      ...
      console.error(
        `❌ Error migrating TopPicks ${pick.title}:`,
        error.message
      );
      errorCount++;
    }
  ...
  for (const user of users) {
    try {
      if (user.profilePicture && user.profilePicture.includes("cloudinary")) {
        ...
        const fileName = `users/${user.id}-${getFileNameFromUrl(
          user.profilePicture
        )}`;
        const newUrl = await uploadToS3(
          imageBuffer,
          fileName,
          getContentType(user.profilePicture)
        );
