...
const scrapeNewsSource = async (source: NewsSource): Promise<ScrapedNews[]> => {
  ...
  return articles.filter(
    (article) => article.title.length > 10 && article.url.startsWith("http")
  );
};

// Function to provide default images when scraping fails
const getDefaultImageForCategory = (
  category: string,
  sourceName: string
): string => {
  const defaultImages = {
...
const fetchImageFromArticlePage = async (
  articleUrl: string
): Promise<string | null> => {
  try {
  ...
  try {
    ...
    for (const source of newsSources) {
      try {
        ...
        console.log(
          `✅ Scraped ${scrapedNews.length} articles from ${source.name}`
        );
    ...
    for (const article of topNews) {
      try {
        ...
        console.warn(
          `Failed to insert article: ${article.title}`,
          dbError.message
        );
      }
    }
    ...
    console.log(
      `✅ Successfully refreshed ${insertedNews.length} trending news articles in ${processingTime}ms`
    );
...
const scoreNewsArticles = (articles: ScrapedNews[]): ScrapedNews[] => {
  return articles
    .map(article => ({
      ...article,
...
const calculateTrendingScore = (article: ScrapedNews): number => {
  ...
  if (article.source_name === 'Variety') score += 10;
  if (article.source_name === 'The Hollywood Reporter') score += 8;
